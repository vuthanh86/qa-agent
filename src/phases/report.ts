...
export interface ReportOptions {
  runDir: string;     // directory containing results.json
  skipHistory?: boolean;
}
...
export interface QaResults {
  ...
  qaAgent: {
    ...
    feature: string;
    env: string;
    checkpoint: { name: string; hashMatched: boolean } | null;
    snapshots: string[];
  };
}
...
export interface QaCase {
  ...
  healed?: boolean;
  healNote?: string;
  a11yAudit?: { violations: number; passes: number };
}
...
export function runReport(options: ReportOptions): ReportResult {
  ...
  if (!raw) throw new Error(`results.json not found in ${options.runDir}`);

  const results: QaResults = JSON.parse(raw);
